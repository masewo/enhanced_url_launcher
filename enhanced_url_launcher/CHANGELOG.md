...
* Fixed an error where 'launch' method of enhanced_url_launcher would cause an error if the provided URL was not valid by RFC 3986.
...
* Require `enhanced_url_launcher_platform_interface` 2.0.3. This fixes an issue
  ...
  of `enhanced_url_launcher_platform_interface`.
...
* Correct statement in description about which platforms enhanced_url_launcher supports.
...
* Fixed a situation where an app would crash if the enhanced_url_launcher’s `launch` method can’t find an app to open the provided url. It will now throw a clear Dart PlatformException.
...
* Use `package:enhanced_url_launcher_platform_interface` to get the platform-specific implementation.
